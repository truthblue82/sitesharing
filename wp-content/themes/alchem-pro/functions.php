...
require get_template_directory() . '/admin/customizer-library/customizer-library.php';
...
require get_template_directory() . '/admin/alchem-customizer.php';

if( !class_exists('sidebar_generator') )
require get_template_directory() . '/lib/multiple_sidebars.php';

/**
 * alchem functions and definitions
 *
 * @package alchem
 */
...
//define('MAGEE_SHORTCODE_LIB_DIR' ,trailingslashit( get_template_directory()) .'lib/shortcodes/');
//define('MAGEE_SHORTCODE_LIB_URI' ,trailingslashit( get_template_directory_uri()) .'lib/shortcodes/');
/**
...
function alchem_setup() {
	global $content_width, $alchem_options,$magee_options, $alchem_upgrade_from_options,$alchem_blog_style;
	$alchem_options = get_theme_mods();
	$magee_options  = $alchem_options;
	...
	$textdomain = alchem_get_textdomain();
	/*
...
//	set_post_thumbnail_size( 860, 200, array( 'center', 'center' ) );
	...
	register_nav_menus( array(
		'primary' => __( 'Primary Menu', 'alchem-pro' ),
		'top_bar_menu' => __( 'Top Bar Menu', 'alchem-pro' ),
		'footer_menu' => __( 'Footer Menu', 'alchem-pro' ),
		'custom_menu_1' => __( 'Custom Menu 1', 'alchem-pro' ),
		'custom_menu_2' => __( 'Custom Menu 2', 'alchem-pro' ),
		'custom_menu_3' => __( 'Custom Menu 3', 'alchem-pro' ),
		'custom_menu_4' => __( 'Custom Menu 4', 'alchem-pro' ),
		'custom_menu_5' => __( 'Custom Menu 5', 'alchem-pro' ),
		'custom_menu_6' => __( 'Custom Menu 6', 'alchem-pro' ),
	) );
	 ...
	 */
	add_theme_support( 'post-formats', alchem_supported_post_formats() );
	...
	// import data from theme options
	if( get_option($textdomain) && $alchem_upgrade_from_options !=1 ){
		alchem_otpions_customize();
		$alchem_upgrade_from_options = 1;
		}
		
	$alchem_free_options     = get_option( "theme_mods_alchem" );
	$alchem_pro_options = get_theme_mods();
	if( is_array( $alchem_free_options ) && $alchem_free_options != null ):
	foreach( $alchem_free_options  as $k => $v ){
		
		if( !isset( $alchem_pro_options[$k] ) && $v !='' ){
...
function alchem_scripts() {
   ...
   if( $body_font ){
	wp_enqueue_style( 'alchem-google-fonts-'.sanitize_title($body_font), 'http' . ( ( is_ssl() ) ? 's' : '' ) . '://fonts.googleapis.com/css?family=' . str_replace(' ','+',$body_font), array(), '' );
   }
   if( $menu_font ){
	wp_enqueue_style( 'alchem-google-fonts-'.sanitize_title($menu_font), 'http' . ( ( is_ssl() ) ? 's' : '' ) . '://fonts.googleapis.com/css?family=' . str_replace(' ','+',$menu_font), array(), '' );
   }
   if( $headings_font ){
	wp_enqueue_style( 'alchem-google-fonts-'.sanitize_title($headings_font), 'http' . ( ( is_ssl() ) ? 's' : '' ) . '://fonts.googleapis.com/css?family=' . str_replace(' ','+',$headings_font), array(), '' );
   }
    if( $footer_headings_font ){
	wp_enqueue_style( 'alchem-google-fonts-'.sanitize_title($footer_headings_font), 'http' . ( ( is_ssl() ) ? 's' : '' ) . '://fonts.googleapis.com/css?family=' . str_replace(' ','+',$footer_headings_font), array(), '' );
   }
   if( $button_font ){
	wp_enqueue_style( 'alchem-google-fonts-'.sanitize_title($button_font), 'http' . ( ( is_ssl() ) ? 's' : '' ) . '://fonts.googleapis.com/css?family=' . str_replace(' ','+',$button_font), array(), '' );
   }
   ...
   if( $home_style == '3' ){
		wp_enqueue_style('alchem-google-fonts-Lato-Cabin-Condensed', esc_url('//fonts.googleapis.com/css?family=Lato|Cabin+Condensed|Poiret+One'), false, '', false );
   }
    ...
    wp_enqueue_style( 'magee-shortcode',  get_template_directory_uri() .'/css/shortcode.css', false, '', false );
	...
	wp_enqueue_style('alchem-scheme',  get_template_directory_uri() .'/css/scheme.less', array('magee-shortcode'),$theme_info->get( 'Version' ), false);
	...
	wp_enqueue_script( 'parallax', get_template_directory_uri() . '/js/jquery.parallax.js', array( 'jquery' ), null, true );
	...
	wp_enqueue_script( 'jquery.countdown', get_template_directory_uri() . '/plugins/jquery-countdown/jquery.countdown.min.js', array( 'jquery' ), null, true );
	wp_enqueue_script( 'jquery.easypiechart', get_template_directory_uri() . '/plugins/jquery-easy-pie-chart/jquery.easypiechart.min.js', array( 'jquery' ), null, true );
	wp_enqueue_script( 'jquery.waypoints', get_template_directory_uri() . '/js/jquery.waypoints.js', array( 'jquery' ), null, true );
	wp_enqueue_script( 'jquery.easing', get_template_directory_uri() . '/js/jquery.easing.min.js' , array( 'jquery' ), null, true);
	wp_enqueue_script( 'jquery.mixitup', get_template_directory_uri() . '/js/jquery.mixitup.min.js', array( 'jquery' ), null, true );
	wp_enqueue_script( 'less', get_template_directory_uri().'/plugins/less.min.js', array( 'jquery' ), '2.5.1', false );
   // wp_register_script( 'google-maps-api', esc_url('//maps.google.com/maps/api/js?sensor=false'), array( 'jquery' ), '', false  );
	wp_enqueue_script( 'respond',  get_template_directory_uri() .'/js/respond.min.js', false, '2.0.0', false );
	wp_enqueue_script( 'jquery.nav',  get_template_directory_uri() .'/js/jquery.nav.js', false, '3.0.0', false );	
	//wp_enqueue_script( 'nicescroll',  get_template_directory_uri() .'/js/smoothscroll.js', false, '', false );	
...
//	if( ! is_singular() ) {
		wp_enqueue_script( 'jquery.infinitescroll', get_template_directory_uri() . '/js/jquery.infinitescroll.js', array( 'jquery' ), null, true );
 //   }
...
// $side_menu_font_size             = absint(alchem_option('side_menu_font_size',13));
   ...
   $woocommerce_icon_font_size    = absint(alchem_option('woocommerce_icon_font_size',14));
	...
	$alchem_custom_css  .= ".post-pagination li a,.post-pagination li span{ font-size:". $pagination_font_size ."px}";
   ...
   // body font
	...
	if( $header_background_color ){
		...
		$rgb = alchem_hex2rgb( $menu_background_color );
	    $alchem_custom_css .= ".main-header .main-header-bottom{
		background-color: rgba(".$rgb[0].",".$rgb[1].",".$rgb[2].",".$menu_background_opacity.");
		}";
...
// page title bar
	...
	for( $i=0;$i<=15;$i++ ){
		...
		if( $section_hide != '1' ){
			...
			$alchem_custom_css  .= "#alchem-home-sections .alchem-home-section-".$i." .home-banner-slider .owl-theme .owl-dots .owl-dot{border: 2px solid ".$color.";}";
...
function alchem_admin_scripts() {
	...
	if( $post_type && $post_type == "magee_slider" ){
	...
	wp_enqueue_script( 'alchem-menu', get_template_directory_uri() . '/js/menu.js',array( 'jquery' ),'' );
	}	
		
	if( $pagenow == "post.php" || $pagenow == "post-new.php" || (isset($_GET['page']) && $_GET['page'] == "options-backup") || (isset($_GET['page']) && $_GET['page'] == "import-demos")){
	wp_enqueue_script( 'alchem-admin', get_template_directory_uri() . '/js/admin.js' ,array( 'jquery' ),'');
	wp_enqueue_style('thickbox');
...
require( trailingslashit( get_template_directory() ) . 'inc/menu-options.php' );
/**
 * Theme breadcrumb
 */
...
//require get_template_directory() . '/lib/shortcodes/shortcodes-core.php';
...
function alchem_deactivate_plugin_conditional() {
...
add_action( 'admin_init', 'alchem_deactivate_plugin_conditional' );
