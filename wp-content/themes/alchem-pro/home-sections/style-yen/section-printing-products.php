...
$args = array ( 'category_name' => 'printing-packaging-supplies', 'orderby' => 'meta_value_num', 'meta_key' => 'ordering', 
...
if($total){
    $section_title = get_cat_name(get_category_by_slug('printing-packaging-supplies')->term_id);
...
?>
    <section class="section magee-section alchem-home-section-4 alchem-home-style-0" id="section-printing-products">
        <div class="section-content container alchem_section_6_model printing-content">
            ...
            <?php 
            $desc = category_description( get_category_by_slug('printing-packaging-supplies')->term_id );
            if($desc){
            ?>
        ...
        $category = get_category(get_category_by_slug('printing-packaging-supplies')->term_id);
        $counter = $category->category_count;
        if($counter > 12){
            ?>
            <div style="text-align: center;padding-bottom:40px;" class="div-printing-more">
                <a class="magee-btn-normal btn-md btn-blue printing-more" href="javascript:void(0);" ><?php echo product_see_more;?></a>
            </div>
            <?php
        }
        ?>
...
<script type="text/javascript">
    jQuery(document).ready(function(){
        jQuery(".printing-more").on("click", function(){
            jQuery(".div-printing-more").html("<img alt='loading' class='loading' src='"+alchem_params.themeurl+"/images/AjaxLoader.gif' />");
            jQuery.ajax({
                url: alchem_params.ajaxurl,
                type: "POST",
                dataType: "html",
                data: {action: "alchem_getPrintingProductMore"},
                success: function(res){
                    jQuery(".printing-content").append(res);
                    jQuery(".div-printing-more").hide();
