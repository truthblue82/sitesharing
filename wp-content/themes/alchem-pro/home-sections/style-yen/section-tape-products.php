<?php 
// section tape products: show 2 rows 8 products
$args = array ( 'category_name' => 'industrial-tapes', 'orderby' => 'meta_value_num', 'meta_key' => 'ordering', 
...
if($total){
    $section_title = get_cat_name(get_category_by_slug('industrial-tapes')->term_id);
    $url_tape = get_permalink(7);
?>

    <section class="section magee-section alchem-home-section-3 alchem-home-style-0" id="section-tape-products">
        <div class="section-content container alchem_section_6_model tape-content">
            <h2 style="text-align: center" class="section-title alchem_section_6_title"><a style="color: #555;" href="<?php echo $url_tape;?>"><?php echo $section_title; ?></a></h2>
            ...
            <?php 
            $desc = category_description( get_category_by_slug('industrial-tapes')->term_id );
            if($desc){
            ?>
        ...
        $category = get_category(get_category_by_slug('industrial-tapes')->term_id);
        $counter = $category->category_count;
        if($counter > 12){
            ?>
            <div style="text-align: center;padding-bottom:40px;" class="div-tape-more">
                <a class="magee-btn-normal btn-md btn-blue tape-more" href="javascript:void(0);" ><?php echo product_see_more;?></a>
            </div>
            <?php
        }
        ?>
...
<script type="text/javascript">
    jQuery(document).ready(function(){
        jQuery(".tape-more").on("click", function(){
            jQuery(".div-tape-more").html("<img alt='loading' class='loading' src='"+alchem_params.themeurl+"/images/AjaxLoader.gif' />");
            jQuery.ajax({
                url: alchem_params.ajaxurl,
                type: "POST",
                dataType: "html",
                data: {action: "alchem_getTapeProductMore"},
                success: function(res){
                    jQuery(".tape-content").append(res);
                    jQuery(".div-tape-more").hide();
