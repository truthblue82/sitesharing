  ...
  function alchem_validate_setting( $input, $type, $field_id, $wmpl_id = '' ) {
    ...
    if ( 'background' == $type ) {
      ...
      if ( 0 === preg_match( '/^#([a-f0-9]{6}|[a-f0-9]{3})$/i', $input ) ) {
        ...
        add_settings_error( 'alchem', 'invalid_hex', __( 'The Colorpicker only allows valid hexadecimal values.', 'alchem-pro' ), 'error' );
...
function alchem_wpml_register_string( $id, $value ) {
  ...
  if ( function_exists( 'icl_register_string' ) ) {
    ...
    icl_register_string( __( 'Theme Options', 'alchem-pro' ), $id, $value );
 ...
 if ( in_array($field_id ,array('page_layout' ,'default_page_layout','default_page_layout','default_post_layout' , 'default_layout','default_portfolio_layout')) ) {
    $array = array(
      ...
      array(
	   ...
	   array(
        'value'   => 'both',
        'label'   => __( 'Both Sidebar', 'alchem-pro' ),
        'src'     => get_template_directory_uri() . '/images/sidebar_both.png'
      ),
	   
	  array(
        'value'   => 'none',
        'label'   => __( 'Full Width', 'alchem-pro' ),
    ...
    if($field_id == "bg_pattern"){
	  ...
	  for( $i=1; $i<=10; $i++ ){
		  ...
		  $items[] = array(
        'value'   => $i,
        'label'   => sprintf(__( 'Pattern %d', 'alchem-pro' ),$i),
...
if ( ! function_exists( 'alchem_admin_metabox_scripts' ) ) {
  ...
  function alchem_admin_metabox_scripts() {
    ...
    $localized_array = array( 
      ...
      'upload_text'           => apply_filters( 'alchem_upload_text', __( 'Send to admin', 'alchem-pro' ) ),
      'remove_media_text'     => __( 'Remove Media', 'alchem-pro' ),
      'reset_agree'           => __( 'Are you sure you want to reset back to the defaults?', 'alchem-pro' ),
      'remove_no'             => __( 'You can\'t remove this! But you can edit the values.', 'alchem-pro' ),
      'remove_agree'          => __( 'Are you sure you want to remove this?', 'alchem-pro' ),
      'activate_layout_agree' => __( 'Are you sure you want to activate this layout?', 'alchem-pro' ),
      'setting_limit'         => __( 'Sorry, you can\'t have settings three levels deep.', 'alchem-pro' ),
      'delete'                => __( 'Delete Gallery', 'alchem-pro' ), 
      'edit'                  => __( 'Edit Gallery', 'alchem-pro' ), 
      'create'                => __( 'Create Gallery', 'alchem-pro' ), 
      'confirm'               => __( 'Are you sure you want to delete this Gallery?', 'alchem-pro' ),
      'date_current'          => __( 'Today', 'alchem-pro' ),
      'date_time_current'     => __( 'Now', 'alchem-pro' ),
      'date_close'            => __( 'Close', 'alchem-pro' )
    );
  ...
  function magee_slider_settings( $id ) {
    ...
    $settings = apply_filters( 'image_slider_fields', array(
      ...
      array(
        'name'      => 'description',
        'type'      => 'textarea',
        'label'     => __( 'Description', 'alchem-pro' ),
        'class'     => ''
      )
    ), $id );
