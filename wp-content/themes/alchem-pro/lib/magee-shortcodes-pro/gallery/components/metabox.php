...
function magee_register_metabox() {
	...
	foreach ( $post_types as $post_type ) {
		...
		add_meta_box( 'featuredgallerydiv', __( 'Portfolio Gallery', 'magee-shortcodes-pro' ), 'magee_display_metabox', $post_type, $context, $priority );
...
function magee_display_metabox() {
	...
	$selectText    = __( 'Select Images', 'magee-shortcodes-pro' );
	...
	if ( ! empty( $galleryString ) ) {
		...
		$selectText = __( 'Edit Selection', 'magee-shortcodes-pro' );
		$visible    = " visible";
	} 
	...
	<button class="button<?php echo $visible.$oldfix; ?>" id="magee_removeall"><?php _e( 'Remove All', 'magee-shortcodes-pro' ); ?></button>
