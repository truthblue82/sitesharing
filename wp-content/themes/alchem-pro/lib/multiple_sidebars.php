...
class sidebar_generator {
	...
	public static function add_meta_boxes() {
		...
		foreach ( $post_types as $post_type ) {
			add_meta_box( 'sbg_box', esc_html__( 'Sidebar', 'alchem-pro' ), array( 'sidebar_generator', 'edit_form' ), $post_type, 'side' );
		}
	}
	...
	public static function add_sidebar(){
		...
		if ( isset( $sidebars[ $id ] ) ) {
			die("alert('" . esc_html__( 'Widget Section already exists, please use a different name.', 'alchem-pro' ) . "')" );
		}
	...
	public static function remove_sidebar(){
		...
		$no_widget_text = esc_html__( 'No Widget Sections defined.', 'alchem-pro' );

		$id = sidebar_generator::name_to_class( $name );
		if ( ! isset( $sidebars[ $id ] ) ) {
			die( 'alert("' . esc_html__( 'Sidebar does not exist.', 'alchem-pro' ) . '")' );
		}
	...
	public static function admin_menu() {
		add_theme_page( esc_html__( 'Widget Sections', 'alchem-pro' ), esc_html__( 'Widget Sections' , 'alchem-pro'), 'manage_options', 'multiple_sidebars', array( 'sidebar_generator', 'admin_page' ) );
	}

	public static function admin_page() { ?>
		<script>
		function remove_sidebar_link(name,num){
			answer = confirm( "<?php esc_html_e( 'Are you sure you want to remove', 'alchem-pro' ); ?> " + name + "?\n<?php esc_html_e( 'This will remove any widgets you have assigned to this widget section.', 'alchem-pro' ); ?>" );
				if ( answer ) {
					//alert('AJAX REMOVE');
			...
			function add_sidebar_link(){
				var sidebar_name = prompt( "<?php esc_html_e( 'Widget Section Name:', 'alchem-pro' ); ?>","" );
				//alert(sidebar_name);
		...
		<div class="postbox" style="max-width:1719px;">
			<h2 class="hndle ui-sortable-handle" style="padding: 15px 12px; margin: 0;">
				<span><?php esc_html_e( 'Widget Sections', 'alchem-pro' ); ?></span>
			</h2>
			<div class="inside" style="margin-bottom: 0;">
				<table class="widefat page" id="sbg_table">
					<tr>
						<th><?php esc_html_e( 'Widget Section Name', 'alchem-pro' ); ?></th>
						<th><?php esc_html_e( 'CSS Class', 'alchem-pro' ); ?></th>
						<th><?php esc_html_e( 'Remove', 'alchem-pro' ); ?></th>
					</tr>
					<?php
					$sidebars = sidebar_generator::get_sidebars();
					?>
					<?php if ( is_array( $sidebars ) && ! empty( $sidebars ) ) : ?>
						...
						<?php foreach ( $sidebars as $sidebar ) : ?>
							...
							<tr class="<?php echo $alt; ?>">
								...
								<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo $sidebar; ?>',<?php echo $cnt + 1; ?>);" title="<?php esc_html_e( 'Remove this Widget Section', 'alchem-pro' ); ?>"><?php esc_attr_e( 'remove', 'alchem-pro' ); ?></a></td>
							</tr>
							<?php $cnt++; ?>
						<?php endforeach; ?>
					<?php else : ?>
						<tr id="no-widget-sections">
							<td colspan="3"><?php esc_html_e( 'No Widget Sections defined.', 'alchem-pro' ); ?></td>
						</tr>
					<?php endif; ?>
				</table>
				<p class="add_sidebar"><a href="javascript:void(0);" onclick="return add_sidebar_link()" title="<?php _e( 'Add New Widget Section', 'alchem-pro' ); ?>" class="button button-primary"><?php _e( 'Add New Widget Section', 'alchem-pro' ); ?></a></p>
			</div>
		</div>
		<?php
	}
	...
	*/
	...
	public static function edit_form() {
		...
		<div class="pyre_metabox_field">
			...
			<div class="pyre_desc">
				<label><?php esc_html_e( 'Select Sidebar 1:', 'alchem-pro' ); ?></label>
				<p><?php esc_html_e( 'Select sidebar 1 that will display on this page. Choose "No Sidebar" for full width.', 'alchem-pro' ); ?></p>
			</div>
			<div class="pyre_field">
				...
				<?php for ( $i = 0; $i < 1; $i++ ) : ?>
					...
					<select name="sidebar_generator[<?php echo $i; ?>]" style="display: none;">
						<option value="0"<?php echo ( '' == $selected_sidebar[ $i ] ) ? ' selected' : ''; ?>><?php esc_html__( 'WP Default Sidebar', 'alchem-pro' ); ?></option>
						<?php $sidebars = $wp_registered_sidebars; // sidebar_generator::get_sidebars(); ?>
						<?php if ( is_array( $sidebars ) && ! empty( $sidebars ) ) : ?>
							<?php foreach ( $sidebars as $sidebar ) : ?>
								<?php if ( $selected_sidebar[ $i ] == $sidebar['name'] ) : ?>
									<?php if ( 'Blog Sidebar' == $sidebar['name'] || esc_html__( 'Blog Sidebar', 'alchem-pro' ) == $sidebar['name'] ) : ?>
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>" selected><?php esc_html_e( 'Default Sidebar', 'alchem-pro' ); ?></option>
									<?php else : ?>
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>" selected><?php echo esc_html( $sidebar['name'] ); ?></option>
									<?php endif; ?>
								<?php else : ?>
									<?php if ( 'Blog Sidebar' == $sidebar['name'] || esc_html__( 'Blog Sidebar', 'alchem-pro' ) == $sidebar['name'] ) : ?>
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>"><?php esc_html_e( 'Default Sidebar', 'alchem-pro' ); ?></option>
									<?php else : ?>
					...
					<select name="sidebar_generator_replacement[<?php echo $i; ?>]">
						<option value="" <?php echo ( '' == $selected_sidebar_replacement[ $i ] && 'post' != $screen->post_type ) ? ' selected' : ''; ?>><?php esc_html_e( 'No Sidebar', 'alchem-pro' ); ?></option>
						...
						<?php if ( is_array( $sidebar_replacements ) && ! empty( $sidebar_replacements ) ) : ?>
							<?php foreach ( $sidebar_replacements as $sidebar ) : ?>
								<?php if ( '0' == $selected_sidebar_replacement[ $i ] ) : ?>
									<?php $selected_sidebar_replacement[ $i ] = esc_html__( 'Blog Sidebar', 'alchem-pro' ); ?>
								<?php endif; ?>
								...
								<?php else : ?>
									<?php if ( 'Blog Sidebar' == $sidebar['name'] || esc_html__( 'Blog Sidebar', 'alchem-pro' ) == $sidebar['name'] ) : ?>
										...
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>"<?php echo $selected; ?>><?php esc_html_e( 'Default Sidebar', 'alchem-pro' ); ?></option>
									<?php else : ?>
										<option value="<?php echo esc_attr( $sidebar['name'] , 'alchem-pro'); ?>"/><?php echo esc_html( $sidebar['name'] ); ?></option>
									<?php endif; ?>
								<?php endif; ?>
							<?php endforeach; ?>
						<?php endif; ?>
		...
		<div class="pyre_metabox_field">
			<input name="sbg_edit" type="hidden" value="sbg_edit" />
			<div class="pyre_desc">
				<label><?php esc_html_e( 'Select Sidebar 2:', 'alchem-pro' ); ?></label>
				<p><?php esc_html_e( 'Select sidebar 2 that will display on this page. Sidebar 2 can only be used if sidebar 1 is selected.', 'alchem-pro' ); ?></p>
			</div>
			<div class="pyre_field">
				...
				<?php for ( $i = 0; $i < 1; $i++ ) : ?>
					...
					<select name="sidebar_2_generator[<?php echo $i; ?>]" style="display: none;">
						<option value="0"<?php echo ( '' == $selected_sidebar_2[ $i ] ) ? ' selected' : ''; ?>><?php esc_html_e( 'WP Default Sidebar', 'alchem-pro' ); ?></option>
						<?php $sidebars = $wp_registered_sidebars; // sidebar_generator::get_sidebars(); ?>
						<?php if ( is_array( $sidebars ) && ! empty( $sidebars ) ) : ?>
							<?php foreach ( $sidebars as $sidebar ) : ?>
					...
					<select name="sidebar_2_generator_replacement[<?php echo $i; ?>]">
						<option value=""<?php echo ( '' == $selected_sidebar_replacement[ $i ] ) ? ' selected' : ''; ?>><?php esc_html_e( 'No Sidebar', 'alchem-pro' ); ?></option>
						<?php $sidebar_replacements = $wp_registered_sidebars; //sidebar_generator::get_sidebars(); ?>
						<?php if ( is_array( $sidebar_replacements ) && ! empty( $sidebar_replacements ) ) : ?>
							<?php foreach ( $sidebar_replacements as $sidebar ) : ?>
			...
			if ( is_array( $selected_sidebar ) && ! empty( $selected_sidebar ) ) {
				...
				for ( $i = 0; $i < $sizeof_selected_sidebar; $i++ ) {
					if ( '0' == $name && '0' == $selected_sidebar[ $i ] && '0' == $selected_sidebar_replacement[ $i ] ) {
						...
						$did_sidebar = true;
						break;
					} elseif ( '0' == $name && '0' == $selected_sidebar[ $i ] || 'Blog Sidebar' == $selected_sidebar[ $i ] || esc_html__( 'Blog Sidebar', 'alchem-pro' ) == $selected_sidebar[ $i ] ) {
